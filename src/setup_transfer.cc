...
void setup_transfer(
    ...
    Eigen::Map<Eigen::MatrixXd> transfer(transfer_npy.data.data(), 
                                            nBinsGen,      
                                            nBinsReco);
    transfer.transposeInPlace();

    Eigen::VectorXd ones = Eigen::VectorXd::Ones(nBinsGen);
    Eigen::VectorXd sumTransfer = transfer * ones;
    ...
    transfer.array().rowwise() /= genDenom.transpose().array();
    ...
    Eigen::VectorXd forward = transfer * genPure;
    printf("recoPure: %g\n", recoPure.sum());
    printf("forward: %g\n", forward.sum());
    ...
    Eigen::VectorXd unfolded_forward = transfer * unfolded; 
