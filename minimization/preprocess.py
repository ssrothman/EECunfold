...
def rebingen(gen, covgen, transfer):
    ...
    new_covshape[3] = covgensum.shape[3] + 1
    new_covshape[7] = covgensum.shape[7] + 1
...
def rebinreco(reco, covreco, transfer):
    ...
    new_covshape[3] = covrecosum.shape[3] + 1
    new_covshape[7] = covrecosum.shape[7] + 1
    ...
    transfer2 = transfer[:,:,:,2::2,:,:,:,:]
    transfersum = transfer1 + transfer2
    transfer = np.concatenate((transfer0, transfersum), axis=3)
...
def get_arrays(hists, syst, statsplit, 
    ...
    if restrictorder is None:
        restrictorder = tuple(range(gen.shape[2]))
    ...
    if restrictpt is None:
        restrictpt = tuple(range(gen.shape[1]))
    ...
    if restrictorder == 'sum':
        gen = np.sum(gen, axis=2)
        covgen = np.sum(covgen, axis=(2,6))
        transfer = np.sum(transfer, axis=(2,6))
        reco = np.sum(reco, axis=2)
        covreco = np.sum(covreco, axis=(2,6))
    else:
    ...
    if restrictpt == 'sum':
        ...
        covgen = np.sum(covgen, axis=(1,5))
        transfer = np.sum(transfer, axis=(1,5))
        reco = np.sum(reco, axis=1)
        covreco = np.sum(covreco, axis=(1,5))
    else:
        gen = gen[:,restrictpt,:,:]
